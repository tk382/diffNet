...
TF = setDF(TF)
TF = TF[,1]
TF_results = getBM(attributes = c('hgnc_symbol','ensembl_gene_id'), 
                filters = "hgnc_symbol",
                values = TF,
                mart = ensembl)
...
psmb_ind = which(colnames(Y) %in% c(results$ensembl_gene_id))
smallY = Y[,-psmb_ind]
...
ptch_ind = which(colnames(Y) %in% c(results$ensembl_gene_id))
smallY = Y[,-ptch_ind]
s = news = rep(0, ncol(Y)-1)
for (i in 1:(ncol(Y)-1)){
...
```{r}
ptch_ind = which(colnames(Y) %in% results[results$hgnc_symbol=="PTCH1", 2])
...
### genes with the highest scores for PTCH1
...
results = getBM(attributes = c('hgnc_symbol','ensembl_gene_id'),
                ...
                values = colnames(Y[,-ptch_ind])[max_scores$ix[1:5]],
                mart = ensembl)
...
```{r}
tead_ind = which(colnames(newY) %in% TF_results[TF_results$hgnc_symbol=="TEAD3", 2])
...
### genes with the highest scores for TEAD3
...
results = getBM(attributes = c('hgnc_symbol','ensembl_gene_id'),
                filters = "ensembl_gene_id",
                values = colnames(Y[,-tead_ind])[max_scores$ix[1:5]],
                mart = ensembl)
