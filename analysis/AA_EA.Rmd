...
# Constricted analysis of transcription factors
...
and at new gene ZSCAN1

```{r}
i = which(results$hgnc_symbol=="ZSCAN1")
targets = fread(paste0("/Volumes/nicolae-lab/users/tae/TF_targets/Homo-sapiens_theoretical_TF2DNA/", results$hgnc_symbol[i], ".pscan"))
targets = targets[which(targets$p_value == min(targets$p_value)), c(1,2,6,7)]
target_genenames = ref[match(targets$target_name, ref$hgnc_symbol), ]
main_ind = which(colnames(Y) == results$ensembl_gene_id[i])
...
plot(diff_cor_fox$AA ~ diff_cor_fox$EA, cex=0.3, ylim = c(-1,1), xlim = c(-1,1), main='ZSCAN1',
     ylab = 'African Americans',
     xlab = 'European Americans'); abline(0,1,col = 'red')
```
...
```{r}
zscan_ind = which(colnames(Y) %in% results[results$hgnc_symbol=="ZSCAN1", 1])
d = get_degree_c(X$A, Y[,fox_ind], Y[,target_ind])
out = rowSums(bootstrap_c(X$A, B, W))
sum(out > d)
hist(out, 50, xlim = c(0, max(d+10, max(out))), main = "ZSCAN1 permutation test")
abline(v=d, col = 'red')
abline(v=d, col = 'red')
```
