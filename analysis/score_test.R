...
library(data.table)
Y = setDF(fread("/Volumes/nicolae-lab/users/tae/Muscle_Skeletal_selected_exp.txt",
                header=T)) #selected expression of A effect pvalue < 0.1
pairs = read.table("/Volumes/nicolae-lab/users/tae/Muscle_Skeletal_selected_pairs.txt",
...
#### random pairs among genes with ancestry effect ####
s2 = rep(0,1000)
for (i in 1:1000){
  inds = sample(nrow(Y), 2)
  y1 = as.numeric(Y[inds[1], ])
  y2 = as.numeric(Y[inds[2], ])
  s2[i] = get_score(y1, y2, A$A)
}
hist(s2,100)
